...
(define (eval-special-form exp env)
  (cond ((quoted? exp) (text-of-quotation exp))
        ...
        ((while? exp) (eval-while exp env)))) ;; 3e
...
;;Oppgave 3a begynner her =====================================================================


(define (and? args)
  (tagged-list? args 'and))
...
(define (let->lambda exp)
  (make-lambda-exp (let-clause exp) (let-body exp)))


;; Liste med variablene 
...
;;Listen med uttrykkene 

(define (make-expression-list clause)
  (if (null? clause)
      '()
      (let ((exp (cadar clause))
            (rest (cdr clause)))
        (cons exp (make-expression-list rest)))))

;; Returnerer lambdauttrykket 


(define (make-lambda-exp clause body)
  (if (null? clause)
      'false
      (cons (make-lambda (make-var-list clause) body)
            (make-expression-list clause))))



;; Oppgave 3e begynner her ===================================
